...
export function useSupabase(workspaceId: string) {
	...
	// Initialize user and check Supabase configuration (runs once)
	useEffect(() => {
							...
							setNodes((prev) =>
								prev.map((node) =>
									node.id === payload.new.id
										? (payload.new as DiagramNode)
										: node,
								),
							);
						} else if (payload.eventType === "DELETE") {
							setNodes((prev) =>
								prev.filter((node) => node.id !== payload.old.id),
							);
						}
					},
				)
	...
	const initializeUser = async () => {
		...
		const userName =
			localStorage.getItem("userName") || `Guest ${userId.slice(0, 8)}`;
		// Get or generate a stable color for this user
				...
				if (error) {
					...
					if (
						error.message?.includes(
							'relation "public.diagram_nodes" does not exist',
						) ||
						error.code === "PGRST116"
					) {
						...
						console.log(
							"Supabase error, switching to offline mode:",
							error.message,
						);
						setIsOfflineMode(true);
						setNodes((prev) => [...prev, newNode]);
